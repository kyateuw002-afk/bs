...
function AdminProducts() {
  ...
  function fetchProducts() {
    setLoading(true);
    Promise.all([
      api.admin.getProducts().then(r => setProducts(r.data?.products || r.data || [])).catch(console.error),
    ...
    <div className="admin-page">
      ...
      <div className="admin-controls-bar">
        ...
        <div className="admin-filter-group">
          ...
          >
            <option value="all">Toutes les collections ({products.length})</option>
            {categories.map(c => (
          ...
          >
            <div className="admin-modal__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                <div className="admin-modal__icon-badge">
                  {editing ? <Edit size={18} className="text-amber" /> : <Package size={18} className="text-amber" />}
                </div>
                <div>
                  <h3 className="heading" style={{ fontSize: '1.2rem', margin: 0, color: '#FFF' }}>
                    {editing ? 'Modifier la création' : 'Nouvelle création olfactive'}
                  </h3>
                  <div style={{ fontSize: '0.75rem', color: '#8C7868' }}>
                    Spécifiez les caractéristiques, tarifs et stocks
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={handleSave} className="admin-form">
              <div className="form-field">
                ...
                <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} required placeholder="Ex: Thiouraye Royal Dakar" />
              </div>
              <div className="form-field">
                <label>Description olfactive & composition</label>
                <textarea value={form.description} onChange={e => setForm(f => ({...f, description: e.target.value}))} rows={3} placeholder="Notes de tête, résines précieuses, rituel d'utilisation..." />
              </div>
              <div className="form-grid-2">
                <div className="form-field">
                  <label>Prix de vente (FCFA) *</label>
                  <input type="number" value={form.price} onChange={e => setForm(f => ({...f, price: e.target.value}))} required placeholder="15000" />
                </div>
                <div className="form-field">
                  <label>Prix barré (optionnel)</label>
                  <input type="number" value={form.compare_price} onChange={e => setForm(f => ({...f, compare_price: e.target.value}))} placeholder="20000" />
                </div>
              </div>
              <div className="form-grid-2">
                <div className="form-field">
                ...
                <div className="form-field">
                  <label>Collection / Catégorie</label>
              ...
              <label className="admin-checkbox-label">
                ...
                <span>Mettre en avant sur la page d'accueil de la Maison</span>
              </label>
              <div className="admin-form__actions">
                <button type="button" className="btn btn--outline" onClick={() => setShowForm(false)}>Annuler</button>
                <button type="submit" className="btn btn--primary admin-btn-gold">Enregistrer la création</button>
              </div>
            </form>
          </motion.div>
        </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
                ...
                <tr key={p.id} className="admin-table__row">
                  <td>
                    <div className="admin-product-cell">
                      <div className="admin-product-thumb-lux">
                        {p.images?.[0]?.url ? (
                          <img src={getImageUrl(p.images[0].url)} alt={p.name} />
                        ) : (
                          <Package size={20} className="text-amber" strokeWidth={1.5} />
                        )}
                      </div>
                      <div className="admin-product-cell__info">
                        <span className="admin-product-cell__name">{p.name}</span>
                        <span className="admin-product-cell__sub">ID #{p.id}</span>
                      </div>
                    </div>
                  </td>
                  <td>
                    <span className="admin-price-cell">{formatPrice(p.price)}</span>
                    {p.compare_price && (
                      <span className="admin-compare-price">{formatPrice(p.compare_price)}</span>
                    )}
                  </td>
                  <td>
                    <span className={`admin-stock-badge ${Number(p.stock) > 0 ? 'admin-stock-badge--in' : 'admin-stock-badge--out'}`}>
                      <span className="admin-stock-dot" />
                      {Number(p.stock) > 0 ? `${p.stock} en stock` : 'Rupture'}
                    </span>
                  </td>
                  <td>
                    <span className="admin-cat-pill">{p.category_name || 'Non classé'}</span>
                  </td>
                  <td>
                    {p.is_featured ? (
                      ...
                      <span style={{ color: '#6A5A4D', fontSize: '0.8rem' }}>—</span>
                    )}
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <div className="admin-actions-group" style={{ justifyContent: 'flex-end' }}>
                      <button className="admin-action-btn-gold" onClick={() => openForm(p)} title="Modifier le produit">
                        <Edit size={14} />
                        <span>Modifier</span>
                      </button>
                      <button className="admin-action-btn-danger" onClick={() => handleDelete(p.id)} title="Supprimer">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
              ...
              {filteredProducts.length === 0 && (
                <tr>
                  <td colSpan={6} className="admin-table__empty">
                    <Package size={36} className="text-amber" style={{ margin: '0 auto 0.75rem', opacity: 0.6 }} />
...
function AdminOrders() {
  ...
  const filteredOrders = orders.filter(o => {
    ...
    const matchesSearch = num.includes(q) || name.includes(q) || phone.includes(q);
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-page__top-banner">
        <div>
          <div className="admin-page__tag">
            ...
            <span>Flux de Ventes & Expéditions</span>
          </div>
          <h1 className="admin-page__title">Commandes de la Maison</h1>
          <p className="admin-section__subtitle" style={{ marginTop: '0.4rem', fontSize: '0.88rem' }}>
      ...
      <div className="admin-controls-bar">
        <div className="admin-search-wrap">
          <input 
            type="text"
            className="admin-search-input"
            placeholder="Rechercher par N° (#BM-...), client ou téléphone…"
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
          />
        </div>
        <div className="admin-filter-group">
          <div className="admin-status-tabs">
            {[
              ...
              <button 
                key={tab.id} 
                className={`admin-status-tab ${statusFilter === tab.id ? 'admin-status-tab--active' : ''}`}
      ...
      {loading ? (
        <div className="admin-loading">
          <RefreshCw size={22} className="spin-anim text-amber" />
          <span>Actualisation du carnet de commandes…</span>
        </div>
      ) : (
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {filteredOrders.map(o => {
                ...
                return (
                  <tr key={o.id} className="admin-table__row">
                    ...
                    <td>
                      <span className="admin-price-cell">{formatPrice(o.total_amount)}</span>
                    </td>
                    <td>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <button 
                        ...
                        onClick={() => { setSelected(o); setNewStatus(o.status); setNote(''); }}
                      >
                        <Edit size={13} />
                        <span>Gérer statut</span>
                      </button>
                    </td>
                  </tr>
                );
              ...
              {filteredOrders.length === 0 && (
                <tr>
                  <td colSpan={6} className="admin-table__empty">
                    <ShoppingCart size={36} className="text-amber" style={{ margin: '0 auto 0.75rem', opacity: 0.6 }} />
                    <div>Aucune commande ne correspond aux filtres sélectionnés.</div>
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <div className="admin-modal__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#8C7868' }}>
                    Client : {selected.customer_first_name} {selected.customer_last_name} ({selected.customer_phone})
                  </div>
                </div>
              </div>
              <button onClick={() => setSelected(null)} className="admin-modal__close" aria-label="Fermer">×</button>
            </div>
            
            <div className="admin-form">
              <div className="admin-order-modal-summary">
                <div className="admin-order-modal-stat">
                  <span className="admin-order-modal-stat__label">Montant de la commande</span>
                  <span className="admin-order-modal-stat__val">{formatPrice(selected.total_amount)}</span>
                </div>
                <div className="admin-order-modal-stat">
                  <span className="admin-order-modal-stat__label">Statut Actuel</span>
                  <StatusBadge status={selected.status} />
                </div>
              </div>

              <div className="form-field">
                <label>Sélectionner le nouvel état d'acheminement *</label>
                <select value={newStatus} onChange={e => setNewStatus(e.target.value)}>
                  <option value="pending">🟡 En attente de validation</option>
                  <option value="confirmed">🟢 Confirmée</option>
                  <option value="processing">🔵 En cours de préparation / Conditionnement</option>
              ...
              <div className="form-field">
                <label>Note de suivi & message logistique (optionnel)</label>
                <textarea 
                  value={note} 
                  onChange={e => setNote(e.target.value)} 
                  rows={3} 
                  placeholder="Ex : Colis remis au coursier Express Dakar, livraison prévue cet après-midi…" 
                />
              </div>

              <div className="admin-form__actions">
                <button className="btn btn--outline" onClick={() => setSelected(null)}>Annuler</button>
                <button className="btn btn--primary admin-btn-gold" onClick={handleStatusUpdate}>
            ...
            <div className="admin-modal__header">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: '#8C7868' }}>
                    Saisie directe d'une commande passée par téléphone, WhatsApp ou en atelier
                  </div>
                </div>
              </div>
              <button onClick={() => setShowManual(false)} className="admin-modal__close" aria-label="Fermer">×</button>
            </div>
            <form onSubmit={handleManualSubmit} className="admin-form">
              <div className="admin-form__section">
                <div className="admin-form__section-title">Coordonnées du Client</div>
                <div className="form-grid-2">
                  <div className="form-field">
                    <label>Nom complet *</label>
                    <input value={manualForm.customerName} onChange={e => setManualForm(f => ({...f, customerName: e.target.value}))} required placeholder="Ex : Fatou Diop" />
                  </div>
                  <div className="form-field">
                    <label>Téléphone principal *</label>
                    <input value={manualForm.customerPhone} onChange={e => setManualForm(f => ({...f, customerPhone: e.target.value}))} required placeholder="+221 77 000 00 00" />
                  </div>
                </div>
                <div className="form-grid-2">
                  <div className="form-field">
                    <label>Téléphone 2 (optionnel)</label>
                    <input value={manualForm.customerPhone2} onChange={e => setManualForm(f => ({...f, customerPhone2: e.target.value}))} placeholder="+221 78 000 00 00" />
                  </div>
                  <div className="form-field">
                    <label>Zone / Quartier</label>
                    <input value={manualForm.customerCity} onChange={e => setManualForm(f => ({...f, customerCity: e.target.value}))} placeholder="Ex : Almadies, Dakar" />
                  </div>
                </div>
                <div className="form-field">
                  <label>Adresse de livraison complète *</label>
                  <input value={manualForm.customerAddress} onChange={e => setManualForm(f => ({...f, customerAddress: e.target.value}))} required placeholder="Rue, numéro de villa, repère..." />
                </div>
                <div className="form-field">
                  <label>Instructions particulières</label>
                  <textarea value={manualForm.customerNote} onChange={e => setManualForm(f => ({...f, customerNote: e.target.value}))} rows={2} placeholder="Remarques sur la livraison ou emballage cadeau souhaité…" />
                </div>
              </div>

              <div className="admin-form__section">
                <div className="admin-form__section-title">Sélection des Articles</div>
                {manualForm.items.map((item, idx) => (
                  <div key={idx} className="admin-form__item-row">
                    <div className="form-field" style={{ flex: 3 }}>
                      <label>Article #{idx + 1}</label>
                      <select value={item.productId} onChange={e => updateItem(idx, 'productId', e.target.value)} required>
                        <option value="">— Choisir un produit —</option>
                        {products.map(p => (
                          <option key={p.id} value={p.id}>{p.name} ({formatPrice(p.base_price || p.price)})</option>
                        ))}
                      </select>
                    </div>
                    <div className="form-field" style={{ flex: 1, minWidth: '80px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', paddingBottom: '2px' }}>
                      <button type="button" className="admin-action-btn-danger" onClick={() => removeItem(idx)} disabled={manualForm.items.length <= 1} title="Supprimer la ligne">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </div>
                ))}
                <button type="button" className="admin-add-line-btn" onClick={addItem}>
                  <Plus size={15} />
              ...
              <div className="admin-form__actions">
                ...
                <button type="submit" className="btn btn--primary admin-btn-gold" disabled={manualSubmitting}>
                  {manualSubmitting ? 'Enregistrement…' : 'Valider la commande'}
                </button>
              </div>
            </form>
          </motion.div>
